...
public:
    ...
    QString generateGeoScript() const override {
        return buildGeoScript(m_radius, m_meshSize, m_height,m_cx, m_cy, m_cz);
    }

    QString getTypeName() const override { return "Cylinder"; }

private:
    // 修改原有的脚本构建逻辑，加入偏移量
    QString buildGeoScript(double radius, double meshSize, double height,double cx, double cy, double cz) const;
